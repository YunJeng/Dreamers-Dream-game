...
class Enemy(pg.sprite.Sprite):
    ...
    def get_image(self, x, y, width, height):
        ...
        image = pg.transform.scale(image,
                                   (int(rect.width*c.EN_SIZE_MULTIPLIER),
                                    int(rect.height*c.EN_SIZE_MULTIPLIER)))
        return image
    ...
    def update(self, game_info, *args):
        ...
        print(f"[DEBUG] Goomba rect: x={self.rect.x}, y={self.rect.y}, bottom={self.rect.bottom}, height={self.rect.height}")
...
class Goomba(Enemy):
    ...
    def __init__(self, y=c.GROUND_HEIGHT, x=0, season=None, direction=c.LEFT, name='goomba'):
        self.season = season
        Enemy.__init__(self)
    ...
    def setup_frames(self):
     ...
     #winter = 1 , spring = 2 , summer = 3 , autumn = 4
        if self.season == 2: #蝴蝶
            self.frames.append(
                self.get_image(205, 0, 100, 158))
            self.frames.append(
                self.get_image(205, 0, 100, 158))
            self.frames.append(
                self.get_image(35, 0, 100, 158))
            self.frames.append(pg.transform.flip(self.frames[1], False, True))
        elif self.season == 3: #仙人掌
            self.frames.append(
                self.get_image(490, 0, 100, 168))
            self.frames.append(
                self.get_image(490, 0, 100, 168))
            self.frames.append(
                self.get_image(355, 0, 100, 168))
            self.frames.append(pg.transform.flip(self.frames[1], False, True))
        elif self.season == 4: #狐狸
            self.frames.append(
                self.get_image(1230, 5, 130, 128))
            self.frames.append(
                self.get_image(1230, 5, 130, 128))
            self.frames.append(
                self.get_image(1050, 0, 130, 123))
            self.frames.append(pg.transform.flip(self.frames[1], False, True))
        elif self.season == 1: #雪人
            self.frames.append(
                self.get_image(1515, 0, 100, 158))
            self.frames.append(
                self.get_image(1515, 0, 100, 158))
            self.frames.append(
                self.get_image(1395, 0, 100, 158))
            self.frames.append(pg.transform.flip(self.frames[1], False, True))
        print(f"[DEBUG] season={self.season}, loaded {len(self.frames)} frames")
...
class Koopa(Enemy):
    ...
    def setup_frames(self):
        """Sets frame list"""
        self.frames.append(
            self.get_image(800, 0, 150, 158))
        self.frames.append(
            self.get_image(800, 0, 150, 158))
        self.frames.append(
            self.get_image(600, 0, 180, 158))
