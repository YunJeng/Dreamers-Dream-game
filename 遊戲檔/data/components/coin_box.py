...
class Coin_box(pg.sprite.Sprite):
    """Coin box sprite"""
    def __init__(self, x, y, season, contents='coin', group=None):
 ...
 #winter = 1 , spring = 2 , summer = 3 , autumn = 4
    def setup_frames(self):
        ...
        if self.season == 1:
            self.frames.append(
                self.get_image(384, 64, 16, 16))
            self.frames.append(
                self.get_image(400, 64, 16, 16))
            self.frames.append(
                self.get_image(416, 64, 16, 16))
            self.frames.append(
                self.get_image(432, 64, 16, 16))
        if self.season == 2 or self.season == 4:
            self.frames.append(
        ...
        if self.season == 3:
            self.frames.append(
                self.get_image(384, 32, 16, 16))
            self.frames.append(
                self.get_image(400, 32, 16, 16))
            self.frames.append(
                self.get_image(416, 32, 16, 16))
            self.frames.append(
                self.get_image(432, 32, 16, 16))
