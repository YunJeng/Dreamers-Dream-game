...
class Level1(tools._State):
    ...
    def startup(self, current_time, persist):
        ...
        #self.flag = None  # 預設為 None


        self.moving_score_list = []
        self.overhead_info_display = info.OverheadInfo(self.game_info, c.LEVEL)#, self.season
    ...
    def setup_ground(self):
        ...
        ground_rect2 = collider.Collider(3048, c.GROUND_HEIGHT,  670, 60)
    ...
    def setup_pipes(self):
        ...
        pipe1 = collider.Collider(1202, 432, 83, 82)
        pipe2 = collider.Collider(1631, 389, 83, 140)
        pipe3 = collider.Collider(1973, 346, 83, 170)
        pipe4 = collider.Collider(2445, 346, 83, 170)
        pipe7 = collider.Collider(3091, 432, 83, 82)
        pipe5 = collider.Collider(7019, 432, 83, 82)
        pipe6 = collider.Collider(7705, 432, 83, 82)
    ...
    def setup_steps(self):
        ...
        step1 = collider.Collider(5775, 475, 40, 44) #+30-20
        step2 = collider.Collider(5818, 432, 40, 44)
        step3 = collider.Collider(5861, 389, 40, 44)
        step4 = collider.Collider(5904, 346, 40, 176)


        step5 = collider.Collider(6031, 346, 40, 176)
        step6 = collider.Collider(6074, 388, 40, 40)
        step7 = collider.Collider(6117, 432, 40, 40)
        step8 = collider.Collider(6160, 475, 40, 40)

        step9 = collider.Collider(6375, 475, 40, 40)
        step10 = collider.Collider(6418, 432, 40, 40)
        step11 = collider.Collider(6461, 389, 40, 40)
        step12 = collider.Collider(6504, 346, 40, 40)
        step13 = collider.Collider(6547, 346, 40, 176)

        step14 = collider.Collider(6674, 346, 40, 176)
        step15 = collider.Collider(6717, 388, 40, 40)
        step16 = collider.Collider(6758, 432, 40, 40)
        step17 = collider.Collider(6801, 475, 40, 40)

        step18 = collider.Collider(7790, 475, 40, 40)
        step19 = collider.Collider(7833, 432, 40, 40)
        step20 = collider.Collider(7875, 389, 40, 40)
        step21 = collider.Collider(7918, 346, 40, 40)
        step22 = collider.Collider(7961, 303, 40, 40)
        step23 = collider.Collider(8004, 260, 40, 40)
        step24 = collider.Collider(8037, 217, 40, 40)
        step25 = collider.Collider(8090, 174, 40, 40)
        step26 = collider.Collider(8133, 174, 40, 360)

        step27 = collider.Collider(8518, 475, 40, 40)
    ...
    '''def setup_season(self):
        """Sets the season for the level"""
        # 假設你在這裡硬編季節為夏天（3）
        self.season = 2
        # 設定 info 顯示用的季節
        self.overhead_info_display = self.season
        # 重新建立載入畫面文字
        self.overhead_info_display.create_load_screen_labels()'''
    ...
    def setup_bricks(self):
        ...
        brick1  = bricks.Brick(858,  365,None , None,"brick",2) #地圖一格為43
        brick2  = bricks.Brick(944,  365,None , None,"brick",2)
        brick3  = bricks.Brick(1030, 365,None , None,"brick",2)
        brick4  = bricks.Brick(3299, 365,None , None,"brick",3)
        brick5  = bricks.Brick(3385, 365,None , None,"brick",3)
        brick6  = bricks.Brick(3430, 193,None , None,"brick",3)
        brick7  = bricks.Brick(3473, 193,None , None,"brick",3)
        brick8  = bricks.Brick(3516, 193,None , None,"brick",3)
        brick9  = bricks.Brick(3559, 193,None , None,"brick",3)
        brick10 = bricks.Brick(3602, 193,None , None,"brick",3)
        brick11 = bricks.Brick(3645, 193,None , None,"brick",3)
        brick12 = bricks.Brick(3688, 193,None , None,"brick",3)
        brick13 = bricks.Brick(3731, 193,None , None,"brick",3)
        brick14 = bricks.Brick(3901, 193,None , None,"brick",3)
        brick15 = bricks.Brick(3944, 193,None , None,"brick",3)
        brick16 = bricks.Brick(3987, 193,None , None,"brick",3)
        brick17 = bricks.Brick(4030, 365,None , None,"brick",3)
        brick18 = bricks.Brick(4287, 365,None , None,"brick",3)
        brick19 = bricks.Brick(4330, 365,None , None,"brick",3)
        brick20 = bricks.Brick(5058, 365,None , None,"brick",4)
        brick21 = bricks.Brick(5187, 193,None , None,"brick",4)
        brick22 = bricks.Brick(5230, 193,None , None,"brick",4)
        brick23 = bricks.Brick(5273, 193,None , None,"brick",4)
        brick24 = bricks.Brick(5488, 193,None , None,"brick",4)
        brick25 = bricks.Brick(5574, 193,None , None,"brick",4)
        brick26 = bricks.Brick(5617, 193,None , None,"brick",4)
        brick27 = bricks.Brick(5531, 365,None , None,"brick",4)
        brick28 = bricks.Brick(5574, 365,None , None,"brick",4)
        brick29 = bricks.Brick(7202, 365,None , None,"brick",1)
        brick30 = bricks.Brick(7245, 365,None , None,"brick",1)
        brick31 = bricks.Brick(7331, 365,None , None,"brick",1)
    ...
    def setup_coin_boxes(self):
        ...
        coin_box1  = coin_box.Coin_box(685, 365, 2, c.COIN, self.coin_group)
        coin_box2  = coin_box.Coin_box(901, 365, 2, c.MUSHROOM, self.powerup_group)
        coin_box3  = coin_box.Coin_box(987, 365, 2, c.COIN, self.coin_group)
        coin_box4  = coin_box.Coin_box(943, 193, 2, c.COIN, self.coin_group)
        coin_box5  = coin_box.Coin_box(3342, 365, 3, c.MUSHROOM, self.powerup_group)
        coin_box6  = coin_box.Coin_box(4030, 193, 3, c.COIN, self.coin_group)
        coin_box7  = coin_box.Coin_box(4544, 365, 3, c.COIN, self.coin_group)
        coin_box8  = coin_box.Coin_box(4672, 365, 3, c.COIN, self.coin_group)
        coin_box9  = coin_box.Coin_box(4672, 193, 3, c.MUSHROOM, self.powerup_group)
        coin_box10 = coin_box.Coin_box(4800, 365, 3, c.COIN, self.coin_group)
        coin_box11 = coin_box.Coin_box(5531, 193, 4, c.COIN, self.coin_group)
        coin_box12 = coin_box.Coin_box(7288, 365, 1, c.COIN, self.coin_group) 
        #coin_box13 = coin_box.Coin_box(7288, 365, 1, c.COIN, self.coin_group) #倒數第二個coinbox不能用
    ...
    """def setup_flag_pole(self):
        ...
        self.flag = flagpole.Flag(8548, 100)

        pole0 = flagpole.Pole(8548, 97)
        pole1 = flagpole.Pole(8548, 137)
        pole2 = flagpole.Pole(8548, 177)
        pole3 = flagpole.Pole(8548, 217)
        pole4 = flagpole.Pole(8548, 257)
        pole5 = flagpole.Pole(8548, 297)
        pole6 = flagpole.Pole(8548, 337)
        pole7 = flagpole.Pole(8548, 377)
        pole8 = flagpole.Pole(8548, 417)
        pole9 = flagpole.Pole(8548, 450)

        finial = flagpole.Finial(8548, 97)
    ...
    def setup_enemies(self):
        ...
        goomba0 = enemies.Goomba(season=2)
        goomba1 = enemies.Goomba(season=2)
        goomba2 = enemies.Goomba(season=2)
        goomba3 = enemies.Goomba(season=3)
        goomba4 = enemies.Goomba(193, season=3)
        goomba5 = enemies.Goomba(193, season=3)
        goomba6 = enemies.Goomba(season=3)
        goomba7 = enemies.Goomba(season=3)
        goomba8 = enemies.Goomba(season=3)
        goomba9 = enemies.Goomba(season=3)
        goomba10 = enemies.Goomba(x=6100,season=4)
        goomba11 = enemies.Goomba(x=6270,season=4)
        goomba12 = enemies.Goomba(x=6440,season=4)
        goomba13 = enemies.Goomba(x=6610,season=4)
        goomba14 = enemies.Goomba(season=1)
        goomba15 = enemies.Goomba(season=1)
    ...
    def setup_checkpoints(self):
        ...
        check11 = checkpoint.Checkpoint(8547, '11', 5, 6)
        ...
        if checkpoint:
            ...
            '''if checkpoint.name == '11':
                ...
                if not hasattr(self, 'flag'):
                    print("self.flag 沒有定義")
                elif not hasattr(self.flag, 'rect'):
                    print("self.flag.rect 不存在")
                else:
                    print(f"flag.rect.y = {self.flag.rect.y}")
            ...
            if checkpoint.name == '12':
                ...
                #if self.overhead_info_display.time == 0 and self.state == c.IN_CASTLE:
    ...
    """def create_flag_points(self):
        ...
        try:
            ...
            print("🏁 Flag points created:", self.flag_score_total)
        except Exception as e:
            print("❌ ERROR in create_flag_points:", e)"""
    ...
    def check_mario_x_collisions(self):
        ...
        if coin_box:
            ...
            if self.mario.invincible: # 瑪莉歐無敵狀態
                #setup.SFX['kick'].play()
                self.sound_manager.play_sound_effect(self.music_dict['kick'])  # ChucK 播放
                self.game_info[c.SCORE] += 100 # 分數加100
            ...
            elif self.mario.getcrown: # 大瑪莉歐狀態
                #setup.SFX['pipe'].play()
                self.sound_manager.play_sound_effect(self.music_dict['kick'])  # ChucK 播放
                #self.mario.fire = False
                self.mario.y_vel = -1
                self.mario.state = c.BIG_TO_SMALL # 大變小
            ...
            if powerup.name == c.STAR: # 無敵星星
                self.game_info[c.SCORE] += 1000 # 分數加1000
                self.moving_score_list.append(
                    score.Score(self.mario.rect.centerx - self.viewport.x, self.mario.rect.y, 1000))
                self.mario.invincible = True # 無敵狀態
                self.mario.invincible_start_timer = self.current_time # 無敵開始時間 等於 現在時間
            elif powerup.name == c.MUSHROOM: # 蘑菇
                #setup.SFX['powerup'].play()
                self.sound_manager.play_sound_effect(self.music_dict['powerup'])  # ChucK 播放
                self.game_info[c.SCORE] += 1000 # 分數加1000
                self.moving_score_list.append(
                    score.Score(self.mario.rect.centerx - self.viewport.x, self.mario.rect.y - 20, 1000))
                self.mario.y_vel = -1
                self.mario.state = c.SMALL_TO_BIG # 馬力歐狀態 等於 小變大
                self.mario.in_transition_state = True # 馬力歐過渡狀態
                #self.convert_mushrooms_to_fireflowers()
            elif powerup.name == c.LIFE_MUSHROOM: # 加命菇
                self.moving_score_list.append(
                    score.Score(powerup.rect.right - self.viewport.x, powerup.rect.y, c.ONEUP))
                self.game_info[c.LIVES] += 1 # 生命值加1
                setup.SFX['one_up'].play()
            elif powerup.name == c.FIREFLOWER: # 火焰花
                #setup.SFX['powerup'].play()
                self.sound_manager.play_sound_effect(self.music_dict['powerup'])  # ChucK 播放
                self.game_info[c.SCORE] += 1000 # 分數加1000
                self.moving_score_list.append(
                ...
                if self.mario.big and self.mario.fire == False: # 大瑪莉歐 且 不是火焰瑪莉歐
                    self.mario.state = c.BIG_TO_FIRE # 瑪莉歐狀態 等於 大變火焰
                    self.mario.in_transition_state = True # 瑪莉歐過渡狀態
                elif self.mario.big == False: # 不是大瑪莉歐
                    self.mario.state = c.SMALL_TO_BIG # 瑪莉歐狀態 等於 小變大
                    self.mario.in_transition_state = True # 瑪莉歐過渡狀態
                    self.convert_mushrooms_to_fireflowers()
    ...
    def adjust_mario_for_x_shell_collisions(self, shell):
        ...
        if shell.state == c.JUMPED_ON:
            ...
            if self.mario.getcrown and not self.mario.invincible:
    ...
    def adjust_mario_for_y_coin_box_collisions(self, coin_box):
        ...
        if self.mario.rect.y > coin_box.rect.y:
            if coin_box.state == c.RESTING:
                ...
                pass
            #setup.SFX['bump'].play()
            self.sound_manager.play_sound_effect(self.music_dict['bump'])  # ChucK 播放
            self.mario.y_vel = 7
    ...
    def adjust_mario_for_y_brick_collisions(self, brick):
        ...
        if self.mario.rect.y > brick.rect.y:
            if brick.state == c.RESTING:
                if self.mario.getcrown and brick.contents is None:
    ...
    def set_game_info_values(self):
        ...
        elif self.mario.dead == False and self.mario.state == c.WALKING_TO_CASTLE:
            self.next = c.WIN
            self.game_info[c.CAMERA_START_X] = 0
